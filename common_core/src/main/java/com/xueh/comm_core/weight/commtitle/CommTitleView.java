...
import com.xueh.comm_core.weight.commtitle.style.TitleViewLightStyle;
import com.xueh.comm_core.weight.commtitle.style.TitleViewTransparentStyle;
 ...
 * 备注：
 */
public class CommTitleView extends FrameLayout
    ...
    public CommTitleView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CommTitleView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initStyle(AttributeSet attrs) {
        ...
        if (sDefaultStyle == null) {
            sDefaultStyle = new TitleViewLightStyle(getContext());
        }

        final TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.CommTitleView);

        final ITitleBarStyle style;
        // 样式设置
        switch (array.getInt(R.styleable.CommTitleView_barStyle, 0)) {
            case 0x10:
                style = new TitleViewLightStyle(getContext());
                break;
            case 0x30:
                style = new TitleViewTransparentStyle(getContext());
                break;
            default:
                style = CommTitleView.sDefaultStyle;
                break;
        }

        // 标题设置
        if (array.hasValue(R.styleable.CommTitleView_leftTitle)) {
            setLeftTitle(array.getString(R.styleable.CommTitleView_leftTitle));
        }

        if (array.hasValue(R.styleable.CommTitleView_title)) {
            setTitle(array.getString(R.styleable.CommTitleView_title));
        } else {
        ...
        // 图标设置
        if (array.hasValue(R.styleable.CommTitleView_rightTitle)) {
            setRightTitle(array.getString(R.styleable.CommTitleView_rightTitle));
        }

        if (array.hasValue(R.styleable.CommTitleView_leftIcon)) {
            setLeftIcon(getContext().getResources().getDrawable(array.getResourceId(R.styleable.CommTitleView_leftIcon, 0)));
        }else {
            if (array.getBoolean(R.styleable.CommTitleView_backButton, style.getBackIcon() != null)) {
                // 显示返回图标
                setLeftIcon(style.getBackIcon());
            }
        }

        if (array.hasValue(R.styleable.CommTitleView_rightIcon)) {
            setRightIcon(getContext().getResources().getDrawable(array.getResourceId(R.styleable.CommTitleView_rightIcon, 0)));
        }

        // 文字颜色设置
        setLeftColor(array.getColor(R.styleable.CommTitleView_leftColor, style.getLeftColor()));
        setTitleColor(array.getColor(R.styleable.CommTitleView_titleColor, style.getTitleColor()));
        setRightColor(array.getColor(R.styleable.CommTitleView_rightColor, style.getRightColor()));

        // 文字大小设置
        setLeftSize(TypedValue.COMPLEX_UNIT_PX, array.getDimensionPixelSize(R.styleable.CommTitleView_leftSize, (int) style.getLeftSize()));
        setTitleSize(TypedValue.COMPLEX_UNIT_PX, array.getDimensionPixelSize(R.styleable.CommTitleView_titleSize, (int) style.getTitleSize()));
        setRightSize(TypedValue.COMPLEX_UNIT_PX, array.getDimensionPixelSize(R.styleable.CommTitleView_rightSize, (int) style.getRightSize()));

        // 背景设置
        if (!array.hasValue(R.styleable.CommTitleView_leftBackground)) {
            setLeftBackground(array.getDrawable(R.styleable.CommTitleView_leftBackground));
        }else {
            setLeftBackground(style.getLeftBackground());
        }

        if (!array.hasValue(R.styleable.CommTitleView_rightBackground)) {
            setRightBackground(array.getDrawable(R.styleable.CommTitleView_rightBackground));
        }else {
            setRightBackground(style.getRightBackground());
        }

        // 分割线设置
        if (array.hasValue(R.styleable.CommTitleView_lineColor)) {
            setLineDrawable(array.getDrawable(R.styleable.CommTitleView_lineColor));
        }else {
            setLineDrawable(style.getLineDrawable());
        }

        setLineVisible(array.getBoolean(R.styleable.CommTitleView_lineVisible, style.isLineVisible()));
        setLineSize(array.getDimensionPixelSize(R.styleable.CommTitleView_lineSize, style.getLineSize()));
     ...
     * 获取点击监听器
     */
    public OnTitleListener getOnTitleBarListener() {
        return mListener;
    }
     ...
     * 设置点击监听器
     */
    public void setOnTitleBarListener(OnTitleListener l) {
        mListener = l;
    }
     ...
     *                      日间模式：{@link TitleViewLightStyle}
     *                      透明模式：{@link TitleViewTransparentStyle}
     */
    public static void initStyle(ITitleBarStyle style) {
        CommTitleView.sDefaultStyle = style;
    }
}