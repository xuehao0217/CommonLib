package com.xueh.commonlib.ui.view;
...
public abstract class ScrollPickerView<T> extends View {
    ...
    private int oldSelected; // 上一次选中的item下标
    ...
    private int mCenterTransY;
    private boolean clickToSlider = false;
    //是否边界可以点击 回弹
    private boolean isOutRangeCanReturn = false;
    ...
    private boolean isOnTouch = false;//是否正在触摸
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            mCenterTransY = typedArray.getInt(R.styleable.ScrollPickerView_spv_center_item_trans,0);
            ...
            clickToSlider = typedArray.getBoolean(R.styleable.ScrollPickerView_spv_is_click_slider,false);

            mItemMargen = typedArray.getInt(R.styleable.ScrollPickerView_spv_hor_item_margen, 0);
            if(mItemMargen>0)
            {
                mItemMargen = ConvertUtils.dp2px(mItemMargen);
            }
            typedArray.recycle();
        }
    }
    ...
    private void reset() {
        ...
        if (mIsHorizontal) {
            ...
            mItemWidth = (getMeasuredWidth()+mItemMargen) / mVisibleItemCount;
        ...
        if (mCenterItemBackground != null) {
            mCenterItemBackground.setBounds(mCenterX, mCenterY+mCenterTransY, mCenterX + mItemWidth, mCenterY + mItemHeight-mCenterTransY);
        }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mSelectedOnTouch != mSelected) { //前后发生变化
                    ...
                    if(oldSelected!=mSelected)
                    {
                        if (mListener != null) {
                            //非被动选择，执行回掉
                            mListener.onInitiativeSelected(this, mSelected);
                            // 告诉监听器选择完毕
                            mListener.onSelected(this, mSelected);
                        }
                    }
    ...
    /**
     *
     * @param isPassive （是否是被动选择）
     */
    private void notifySelected(boolean isPassive) {
        mMoveLength = 0;
        cancelScroll();
        if(oldSelected!=mSelected)
        {
    ...
    public void autoScrollToPosition(int toPosition, long duration, final Interpolator interpolator) {
        ...
        if(mIsHorizontal)
        {
            endY = (mSelected - toPosition) * mItemWidth;
        }else
        {
            endY = (mSelected - toPosition) * mItemHeight;
        }
            ...
            if (mCanTap && !mIsScrollingLastTime) {
                if (lastMove >= mCenterPoint && lastMove <= mCenterPoint + mItemSize) { //点击中间item，回调点击事件
                    ...
                    if(clickToSlider)
                    {
                        if(isHorizontal())
                        {
                            float distance = mCenterPoint - lastMove;
                            if(distance>mItemSize)
                            {
                                int size = (int) (distance/mItemSize);
                                //取余数计算
                                if(distance%mItemSize>0)
                                {
                                    size+=1;
                                }
                    ...
                    if(!isOutRangeCanReturn)
                    {
                        if(null!=mData && !mData.isEmpty() && mSelected == getData().size()-1)
                        {
                            return true;
                        }
                    }
                    ...
                    if(clickToSlider)
                    {
                        if(isHorizontal())
                        {
                            ...
                            {
                                float distance = lastMove-mCenterPoint;
                                if(distance>mItemSize)
                                {
                                    int size = (int) (distance/mItemSize);
                                    if(size>1)
    ...
    public void setData(List<? extends T> data,int selectPosition) {
        if (data == null) {
            mData = new ArrayList<T>();
        } else {
            this.mData = (List<T>) data;
        ...
        //传进来的默认选中
        if(selectPosition>=0 && selectPosition<mData.size())
        {
            mSelected = selectPosition;
        }else
        {
            mSelected = mData.size() / 2;
        }
    ...
    public interface OnSelectedListener {
        ...
        void onInitiativeSelected(ScrollPickerView scrollPickerView, int position);//˙主动选择
    }
