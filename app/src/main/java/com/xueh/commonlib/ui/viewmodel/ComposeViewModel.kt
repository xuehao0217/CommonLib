...
class ComposeViewModel : BaseRequstViewModel<RestApi>() {
    ...
    fun loadDsl() {
        apiDSL {
            onRequest {
                val r = api.bannerList3()
                if (!r.isSuccess()) {
                    throw Exception(r.errorMsg.ifBlank { "errorCode=${r.errorCode}" })
                }
                r.data
            }
            onResponse {
    ...
    /**
     * 分页：直连玩 Android `article/list/{page}/json`。
     * 若需模拟失败，在单测或单独开关里 throw，勿在 catch 中再次抛固定文案（会掩盖真实网络/解析错误）。
     */
    fun getListDatas() = pager { pageNumber ->
        delay(300)
        val result = api.getHome(pageNumber)
        if (!result.isSuccess()) {
            throw Exception(result.errorMsg.ifBlank { "errorCode=${result.errorCode}" })
        }
        when (pageNumber) {
            6 -> emptyList()
            else -> result.data.datas
        }
    }
