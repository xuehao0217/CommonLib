...
| **app** | Application | **单 Activity**（`MainActivity`）：闪屏与底部导航同进程；全屏 Web/测试页为覆盖层。首页为 Navigation 3 示例列表；「我的」「拦截」Tab 为调试与外链 |
...
- **SDK**：`compileSdk` / `targetSdk` **36**，`minSdk` **24**（与 `compose-webview-android` 等依赖一致）
...
│       └── ui/               # MainActivity、HomePage、AppShell（全屏覆盖）、各 Compose 示例页
├── common_core/              # 基础库
...
安装：在 Android Studio 中选择 **`app`** 运行，启动 Activity 为 **`MainActivity`**（`LAUNCHER`）；闪屏在 Compose 内展示后进入主导航。`app` 的 Manifest 通过 `tools:node="remove"` 移除了库中合并进来的 `AgentComposeWebActivity`，演示工程内嵌 Web 使用 `AgentWebScaffold`。
